...
:: Set layer settings
echo.
echo 正在应用兼容性选项，MAJOR=%MAJOR%, MINOR=%MINOR%...
...
reg add "HKCR\Applications\Viewer.exe" /v "FriendlyAppName" /t REG_SZ /d "Windows XP 图片和传真查看器" /f >nul
...
reg add "HKEY_CLASSES_ROOT\Local Settings\Software\Microsoft\Windows\Shell\MuiCache" /v "!ESCAPED_DIR!\\Viewer.exe" /t REG_SZ /d "Windows XP 图片和传真查看器" /f
pause